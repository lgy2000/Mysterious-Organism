...
function pAequorFactory(specimenNum,dna){
  let obj={
    ...
    complementStrand(){
      let q=[]
      for(let r=0;r<15;r++){
        if(this.dna[r]=='A'){q.push('T')}
        else if(this.dna[r]=='T'){q.push('A')}
        else if(this.dna[r]=='C'){q.push('G')}
        else if(this.dna[r]=='G'){q.push('C')}
      }
...
function thirtypAequor(){
  ...
  for(let o=1;o<=30;o++){/////////******** */
    let obj1
    do{obj1=pAequorFactory(o,mockUpStrand())
...
function mostRelated(arr1){
  ...
  let arr3=arr2.sort((a,b)=>{return b.compareDNA-a.compareDNA})
  return arr3[0]
}
